...
CMD socat tcp6-listen:$LOCAL_PORT,reuseaddr,fork tcp:$REMOTE_HOST:$REMOTE_PORT & pid=$! && trap "kill $pid" SIGINT && \
	echo "Socat started listening on IPv6 $LOCAL_PORT: Redirecting traffic to IPv4 $REMOTE_HOST:$REMOTE_PORT ($pid)" && wait $pid
